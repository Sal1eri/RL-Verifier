...
WANDB_NAME="${WANDB_NAME:-qwen3-4b-rule-v100x4-safe}" \
PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}" \
uv run accelerate launch --num_processes 4 train_grpo.py \
    --dataset-path ./mydata/dapo_math.jsonl \
    --reward-func accuracy \
    --output-dir ./outputs/qwen3_4b_rule_v100x4_safe \
    ...
    --logging-steps 1 \
    --report-to wandb \
    --run-name qwen3-4b-rule-v100x4-safe \
